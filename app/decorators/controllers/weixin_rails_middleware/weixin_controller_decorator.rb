...
WeixinRailsMiddleware::WeixinController.class_eval do
  ...
  private
    ...
    def response_location_message(options={})
      ...
      tmp = @label.scan(/(省|州)/)
      if not tmp.blank?
        sublabel = @label.match(/#{tmp.last[0]}(\S*[市])/)[1]
      else
        sublabel = @label.match(/(\S*[市])/)[1]
      end
      url = "http://www.sojson.com/open/api/weather/json.shtml?city=#{sublabel}"
      resp = RestClient.get(URI::encode(url))
      respbody = JSON.parse(resp.body)
      jrtq = respbody["data"]["forecast"][0]
      tq = "日期：#{jrtq["date"]}\n日出：#{jrtq["sunrise"]}\n最高温：#{jrtq["high"]}\n最低温：#{jrtq["low"]}\n日落：#{jrtq["sunset"]}\n风向：#{jrtq["fx"]}\n风力：#{jrtq["fl"]}\n天气：#{jrtq["type"]}\n注意：#{jrtq["notice"]}"
      reply_text_message("您的位置为: #{@lx}, #{@ly}, #{@scale}, #{@label}\n您所在位置的天气为：\n#{tq}")
    end
    ...
    def handle_click_event
      if @keyword == "tianqi"
      url = "http://www.sojson.com/open/api/weather/json.shtml?city=昆明"
      resp = RestClient.get(URI::encode(url))
      respbody = JSON.parse(resp.body)
      jrtq = respbody["data"]["forecast"][0]
      tq = "日期：#{jrtq["date"]}\n日出：#{jrtq["sunrise"]}\n最高温：#{jrtq["high"]}\n最低温：#{jrtq["low"]}\n日落：#{jrtq["sunset"]}\n风向：#{jrtq["fx"]}\n风力：#{jrtq["fl"]}\n天气：#{jrtq["type"]}\n注意：#{jrtq["notice"]}"
      reply_text_message("#{tq}")
